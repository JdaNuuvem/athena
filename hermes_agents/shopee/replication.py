...
def _clonar_item(item: dict, destino_loja_id: int) -> dict:
    """Clona 1 item (dict do get_item_base_info) para destino_loja_id.
    Retorna {"sucesso": bool, "sku": str, "mensagem": str}."""
    ...
    for img in (item.get("image", {}) or {}).get("image_id_list", []) or (item.get("images") or []):
        ...
        try:
            ...
            continue
    if not image_ids:
        return {"sucesso": False, "sku": sku, "mensagem": f"Item #{item_id} ({sku}): sem imagens"}
    if not cat_id:
        return {"sucesso": False, "sku": sku, "mensagem": f"Item #{item_id} ({sku}): sem categoria"}

    margem = calcular_margem_produto(sku, float(preco), loja_id=destino_loja_id)
    if not margem["ok"]:
        return {"sucesso": False, "sku": sku, "mensagem": f"Item #{item_id} ({sku}) NAO publicado — {margem['mensagem']}"}
    ...
    if resp_add.get("error"):
        return {"sucesso": False, "sku": sku, "mensagem": f"Item #{item_id} ({sku}): {resp_add['error']}"}

    mensagem = f"Item #{item_id} ({sku}): publicado com sucesso"
    ...
    return {"sucesso": True, "sku": sku, "mensagem": mensagem, "item": resp_add}


def clonar_item_para_loja(item_id: int, origem_loja_id: int, destino_loja_id: int) -> dict:
    """Clona 1 produto ja publicado na loja origem para a loja destino."""
    ...
    try:
        info = get_item_base_info([item_id], loja_id=origem_loja_id)
        item_list = info.get("response", {}).get("item_list", []) if isinstance(info, dict) else []
    except Exception as e:
        return {"error": f"Falha ao buscar item {item_id}: {e}"}
    if not item_list:
        return {"error": "Item nao encontrado na loja origem"}
    try:
        return _clonar_item(item_list[0], destino_loja_id)
    except Exception as e:
        return {"error": str(e)}
    ...
    for i in range(0, len(todos_ids), batch_size):
        ...
        for item in item_list:
            ...
            try:
                ...
                resultado_item = _clonar_item(item, destino_loja_id)
                if resultado_item["sucesso"]:
                    resultados["sucesso"] += 1
                    if "mas" in resultado_item["mensagem"] or "—" in resultado_item["mensagem"]:
                        resultados["erros"].append(resultado_item["mensagem"])
                else:
                    resultados["erros"].append(resultado_item["mensagem"])
                    if "NAO publicado" in resultado_item["mensagem"]:
                        resultados["pulados"] += 1
            except Exception as e:
                resultados["erros"].append(f"Item #{item_id} ({sku or '?'}): {e}")
