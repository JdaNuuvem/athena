...
export default function Page() {
  ...
  const handleCreate = async () => {
    await fetch("/api/compras/notas", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify(form) });
    setShowForm(false); setForm({}); carregar();
  };
  const handleDelete = async (id: number) => {
    if (!confirm("Remover?")) return;
    await fetch("/api/compras/notas/" + id, { method: "DELETE" });
    carregar();
  };
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-neutral-100">Notas Entrada</h1><p className="text-xs text-neutral-500 mt-1">Notas fiscais de entrada</p></div>
      ...
      {showForm && (
        <div className="bg-neutral-800 border border-neutral-700 rounded-lg p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {["pedido_id","numero_nf","valor","data_emissao","status"].map(c => (
        ...
        <div className="bg-neutral-800 border border-neutral-700 rounded-lg overflow-hidden">
          <table className="w-full text-xs">
            <thead><tr className="border-b border-neutral-700 text-neutral-400 bg-neutral-850">
              {["pedido_id","numero_nf","valor","data_emissao","status"].map(c => <th key={c} className="text-left p-3 capitalize">{c.replace(/_/g," ")}</th>)}
