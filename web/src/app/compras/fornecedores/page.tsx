...
const CAMPOS = ["nome", "documento", "tipo", "status", "score"];
const COLUNAS = ["nome", "documento", "tipo", "status", "score"];
...
export default function Page() {
  ...
  const carregar = () => {
    // Unificado: usa cad_fornecedores como fonte central
  ...
  const handleCreate = async () => {
    const sender = { ...form, tipo: form.tipo || "produto" };
    await fetch("/api/cadastros/fornecedores", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify(sender) });
    setShowForm(false); setForm({}); carregar();
  };
  const handleDelete = async (id: number) => {
    if (!confirm("Remover?")) return;
    await fetch("/api/cadastros/fornecedores/" + id, { method: "DELETE" });
    carregar();
  };
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-neutral-100">Fornecedores</h1>
          <p className="text-xs text-neutral-500 mt-1">Cadastro unificado — compartilhado com Cadastros</p>
        </div>
      ...
      {showForm && (
        <div className="bg-neutral-800 border border-neutral-700 rounded-lg p-4 space-y-3">
          <h3 className="text-sm font-semibold text-neutral-200">Novo Fornecedor</h3>
      ...
      {loading ? <p className="text-neutral-500 text-sm">Carregando...</p> : items.length === 0 ? (
        <div className="bg-neutral-800 border border-neutral-700 rounded-lg p-8 text-center"><p className="text-neutral-400 text-sm">Nenhum fornecedor</p></div>
      ) : (
        <div className="bg-neutral-800 border border-neutral-700 rounded-lg overflow-hidden">
          <table className="w-full text-xs">
            <thead><tr className="border-b border-neutral-700 text-neutral-400 bg-neutral-850">
              {COLUNAS.map(c => <th key={c} className="text-left p-3 capitalize">{c.replace(/_/g," ")}</th>)}
            ...
            <tbody>{items.map((item, i) => (
              <tr key={item.id} className={"border-b border-neutral-700/50 " + (i%2===0?"bg-neutral-800":"bg-neutral-800/50")}>
                {COLUNAS.map(c => (
                  <td key={c} className="p-3 text-neutral-300">{String(item[c] ?? "—")}</td>
                ))}
