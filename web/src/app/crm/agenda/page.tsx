...
export default function Page() {
  ...
  const handleCreate = async () => {
    await fetch("/api/crm/agenda", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify(form) });
    setShowForm(false); setForm({}); carregar();
  };
  const handleDelete = async (id: number) => {
    if (!confirm("Remover?")) return;
    await fetch("/api/crm/agenda/" + id, { method: "DELETE" });
    carregar();
  };
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-neutral-100">Agenda</h1><p className="text-xs text-neutral-500 mt-1">Atividades e follow-ups</p></div>
      ...
      {showForm && (
        <div className="bg-neutral-800 border border-neutral-700 rounded-lg p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {["tipo","descricao","data_agendada","status"].map(c => (
        ...
        <div className="bg-neutral-800 border border-neutral-700 rounded-lg overflow-hidden">
          <table className="w-full text-xs">
            <thead><tr className="border-b border-neutral-700 text-neutral-400 bg-neutral-850">
              {["tipo","descricao","data_agendada","status"].map(c => <th key={c} className="text-left p-3 capitalize">{c.replace(/_/g," ")}</th>)}
