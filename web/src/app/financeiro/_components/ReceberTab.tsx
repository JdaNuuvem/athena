  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[{ label: "Total Pendente", v: data.filter(c => c.status !== "pago").reduce((s, c) => s + c.valor, 0), c: "text-amber-400" }, { label: "Total Recebido", v: data.filter(c => c.status === "pago").reduce((s, c) => s + c.valor, 0), c: "text-emerald-400" }, { label: "Vencidas", v: data.filter(c => c.status !== "pago" && c.vencimento && new Date(c.vencimento) < new Date(new Date().toDateString())).length, c: "text-red-400" }, { label: "Total", v: data.length, c: "text-neutral-200" }].map((c) => (
