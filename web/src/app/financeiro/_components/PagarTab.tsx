...
export default function PagarTab() {
  ...
  useEffect(() => { api.finList("contas_pagar").then((r) => setData((r.data || []) as Conta[])).catch(() => {}).finally(() => setLoading(false)); }, []);
      ...
      {loading ? <p className="text-xs text-neutral-500">Carregando...</p> : (
        <div className="bg-neutral-800 border border-neutral-700 rounded-lg overflow-hidden"><table className="w-full text-xs">
          ...
          <tbody>{data.map((c) => {
            ...
            return <tr key={c.id} className="border-b border-neutral-700/50 hover:bg-neutral-700/30 text-neutral-300"><td className="px-4 py-2.5">{c.fornecedor}</td><td className="px-4 py-2.5">{c.descricao}</td><td className="px-4 py-2.5">{fmt(c.valor)}</td><td className="px-4 py-2.5">{c.vencimento ? new Date(c.vencimento + "T00:00:00").toLocaleDateString("pt-BR") : "—"}</td><td className="px-4 py-2.5">{c.forma_pagamento}</td><td className="px-4 py-2.5"><span className={`px-2 py-0.5 rounded text-[10px] ${sc}`}>{c.status}</span></td></tr>;
          })}</tbody>
        </table></div>
      )}</div>
  );
