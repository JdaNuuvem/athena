...
export default function Page() {
  ...
  return (<div className="p-6 space-y-4">
    ...
    {loading?<p className="text-neutral-500 text-sm">Carregando...</p>:items.length===0?<p className="text-neutral-500">Nenhum vale</p>:(
      <div className="bg-neutral-800 border border-neutral-700 rounded-lg overflow-x-auto"><table className="w-full text-xs"><thead><tr className="border-b border-neutral-700 text-neutral-400 bg-neutral-850">{["Func ID","Nome","Valor","Motivo","Data","Status"].map(c=><th key={c} className="text-left p-2">{c}</th>)}<th className="text-right p-2">Acao</th></tr></thead><tbody>{items.map((v,i)=>(<tr key={v.id} className={"border-b border-neutral-700/50 "+(i%2===0?"bg-neutral-800":"bg-neutral-800/50")}><td className="p-2">{v.funcionario_id}</td><td className="p-2 text-neutral-200">{v.nome}</td><td className="p-2 text-emerald-400">R$ {(v.valor||0).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})}</td><td className="p-2 text-neutral-400">{v.motivo}</td><td className="p-2 text-neutral-500">{fmtDataBR(v.data)}</td><td className="p-2"><span className={"px-1.5 py-0.5 rounded text-[9px] "+(v.status==="pago"?"text-emerald-400 bg-emerald-900/30":"text-amber-400 bg-amber-900/30")}>{v.status||"pendente"}</span></td><td className="p-2 text-right">{v.status!=="pago"&&<button onClick={()=>handlePagar(v.id)} className="text-emerald-400 hover:text-emerald-300 text-xs">Pagar</button>}</td></tr>))}</tbody></table></div>)}</div>);
}
