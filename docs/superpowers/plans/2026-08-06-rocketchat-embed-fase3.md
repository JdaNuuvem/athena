...
6. **Limitação conhecida da checagem de saúde (considerar habilitar CORS depois de validado):** a checagem de saúde do frontend (`RocketChatFrame.tsx`) usa `fetch` em modo `no-cors` contra `/api/v1/info` — isso confirma que o host responde, mas não distingue "Rocket.Chat de pé" de "proxy retornando 502/503 porque o container ainda está subindo" (o `docker-compose.yml` tem `start_period: 120s` no healthcheck). Uma vez a infra provisionada e validada, considere habilitar CORS no Rocket.Chat (`API_Enable_CORS` + `API_CORS_Origin: https://athena.zoikom.site`) e trocar o `fetch` do frontend de `mode: "no-cors"` para `mode: "cors"` checando `res.ok`, pra virar uma checagem de saúde de verdade em vez de só uma checagem de alcançabilidade de rede.
7. Acessar `https://athena.zoikom.site/chat` em produção (com as Tasks 1+2 já deployadas) e confirmar: estado deixa de ser "indisponível" sozinho (ou após clicar "Tentar novamente"), o iframe carrega, o botão "Hermes" aparece dentro dele, login funciona, e uma chamada de vídeo abre (valida o `allow=` do iframe).
