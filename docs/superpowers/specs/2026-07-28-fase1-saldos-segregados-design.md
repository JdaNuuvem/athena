...
- `solicitar()`: em vez de `_debitar_origem` direto em `estoque_lojas`, chama `mover_saldo(sku, origem, 'disponivel', 'transito', quantidade, 'transferencia_saida', motivo, usuario_id, usuario_nome)`. Quantidade fica visível como saldo `transito`, não desaparece.
- `aprovar()`: só muda `status`, não mexe em saldo (já é assim hoje).
- `rejeitar()`: **bug adicional encontrado** — hoje só muda `status` para `rejeitada`, nunca devolve o saldo debitado em `solicitar()`. Estoque fica perdido permanentemente em qualquer rejeição. Corrigido: chama `mover_saldo(sku, origem, 'transito', 'disponivel', quantidade_solicitada, 'estorno', motivo_rejeicao, aprovador_id, aprovador_nome)` antes de atualizar o status.
